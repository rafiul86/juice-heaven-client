...
const DeleteProducts = () => {
    ...
    return (
            <div>
            {
                orders.map(order => <li>Product :  {order.name} , Price :     {order.price}<Button onClick={()=>handleDelete(order._id)} color="secondary" variant="contained"><DeleteForeverIcon/></Button>  </li> )
            }
        </div>
    );
};
